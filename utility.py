...
def update_carbon_masses(self):
    ...
    _b21, _b22, _b23 = 0.097213, 0.852787, 0.05
    _b31, _b32, _b33 = 0, 0.003119, 0.996881
    self.carbon_matrix = np.array(
        [_b11, _b12, _b13, _b21, _b22, _b23, _b31, _b32, _b33,]
    ).reshape(3, 3)
...
def emmision_control_from_carbontax(self, carbon_tax):
    ...
    if self.time <= 10 and self.time > 5:
        output = np.power(
            t5 / self.BC[-1], 1 / (self.exponent_emission_reduction_theta2 - 1)
        )
    elif self.time <= 15 and self.time > 10:
        output = np.power(
            t10 / self.BC[-1], 1 / (self.exponent_emission_reduction_theta2 - 1)
        )
    elif self.time > 15:
        output = np.power(
            t15 / self.BC[-1], 1 / (self.exponent_emission_reduction_theta2 - 1)
        )
    else:
        output = 1
